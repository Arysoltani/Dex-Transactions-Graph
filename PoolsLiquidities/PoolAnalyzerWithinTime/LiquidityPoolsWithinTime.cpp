	...
	vector<pair<pair<string, string>, pair<string, PoolInfo*> > >& sorted_pools)
{
	...
	output_stream << "PoolAddress,FirstBlock,LastBlock,LiquidityToken0,LiquidityToken1,AddressToken0,SymbolToken0,AddressToken1,SymbolToken1,";
	
	for(int i = 0; i < num_blocks; i++)
	{
		output_stream << "count" + int_to_string(i);
		if(i != (num_blocks - 1))
			output_stream << ",";
		else 
			output_stream << endl;
	...
	for(int i = 0; i < sorted_pools.size(); i++)
	{
		...
		else
		{
			output_stream << sorted_pools[i].second.first << "," << pool_now->first_block << "," <<
				...
				<< pool_now->token2.name << ",";
			for(int j = 0; j < pool_now->count_transfer.count_transfers.size(); j++)
			{
				output_stream << pool_now->count_transfer.count_transfers[j];
				if(j != (int)pool_now->count_transfer.count_transfers.size() - 1)
					output_stream << ",";
				else 
					output_stream << endl;
			}
